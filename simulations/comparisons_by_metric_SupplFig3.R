...
for (metric in metrics){
  results[[metric]] = data.frame(matrix(nrow = length(dataset_name),
                                        ...
                                        dimnames = list(dataset_name,
                                                        c(paste0("X",c(1:10, 12)), "average"))))
  for (X in c(1:10, 12)){
    for (data in 1:length(dataset_name)){
      if (data == 2) {suffix <- "_all_PERC"} else {suffix <- ""}
      input_s <- inputs[[data]]
      tryCatch(
        expr={
          # Cada archivo se compara con las simulaciones neutras
          simul <- read.csv(paste0(input_s,"/data_simul_",metric,"_X",X,suffix,".csv"),
          ...
          # row.names = 1, check.names = F)
          real <- read.csv(paste0(input_real, "/", "data_real_", metric,"_X",X,".csv"),
                           row.names = 1, check.names = F)
          # NA handling
          if (sum(is.na(real))>0){
            message(paste("A total of",sum(is.na(real)),"NAs at real dataset //",metric, X, dataset_name[data]))
          }
          real[is.na(real)] <- 0
          
          if (sum(is.na(simul))>0){
            message(paste("A total of",sum(is.na(simul)),"NAs at simul dataset //",metric, X, dataset_name[data]))
          }
          simul[is.na(simul)] <- 0
          ...
          if (r[["P-Value"]] < 0.05) {
            results[[metric]][dataset_name[data], c(paste0("X",X))] <- round(r[["Test Stat"]],r_dig)
          } else {
            results[[metric]][dataset_name[data], c(paste0("X",X))] <- 0 # <- lo hago 0 porque si no es significativo quiere decir que son la misma distribución (aparentemente al menos)
            # paste0("(",round(r[["Test Stat"]],r_dig),")")
          }
        },
        error=function(cond){
          message(paste("ERROR at", metric, X, dataset_name[data], cond))
        }
      )
